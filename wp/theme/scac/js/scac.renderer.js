...
SCAC.Renderer = SCAC.Renderer || function(config) {
		...
		/**
        *	Rendered: constructor for objects that loads .json, fetch template, render content and appned to the dom.
	...
	var templateName = config.templateName || '',
    	...
    	fetch = function() {
	    	$.ajax({
                    url: SCAC.router.pathTo('templates/' + config.templateURL),
                    type: 'POST',
                    dataType: 'text'        
            })
